...
| 유형 | 역할 | 위치 (VS Code) |
|------|------|----------------|
| **디스패처** (Dispatcher) | LLM이 매 요청마다 자동으로 읽는 22줄 안내 | `.github/copilot-instructions.md` |
...
kode:musher에는 3가지 부품이 있습니다. **이 구분을 이해하면 언제 뭘 써야 하는지 명확해집니다.**

### 디스패처 (Dispatcher) — 자동. 사용자가 신경 쓸 필요 없음

LLM이 **매 요청마다 알아서** 읽는 22줄 파일입니다. 호출하지 않아도 적용됩니다.
...
```
1. npx musher-engineering init --ide vscode
   → 프로젝트 언어를 자동 감지
2. "bootstrap 실행해줘"
...
kode:musher의 핵심입니다. LLM이 세션 간에 프로젝트를 기억하는 유일한 방법입니다.
...
### Q: BMAD와 어떻게 다른가?
**A**: kode:musher는 LLM 컨텍스트 최소화에 집중합니다:
- BMAD: 200+ 파일, 34+ 워크플로우 → 강력하지만 저성능 LLM에서 컨텍스트 오버플로우 위험
- kode:musher: ~27 파일, 8 스킬 → 저성능 LLM(GPT-OSS-120B 등)에서도 안정 동작

### Q: 다른 IDE에서도 쓸 수 있나?
**A**: 네. 6개 IDE를 지원합니다:
```
npx musher-engineering init --ide <vscode|claude|cursor|codex|windsurf|antigravity>
```
...
팀 단위로 kode:musher를 사용할 때는 `--team` 플래그로 초기화합니다:

```bash
npx musher-engineering init --ide vscode --team
```
