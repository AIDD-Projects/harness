...
설치 직후:
```
"bootstrap 실행해줘" → 상태 파일 자동 채우기 → 개발 시작
```
...
- `copilot-instructions.md` → Iron Laws, Direction Guard, Session Bootstrap
- `testing.instructions.md` → 테스트 코드 작성 시 자동 적용
- `backend.instructions.md` → 백엔드 코드 작성 시 자동 적용

**사용자 할 일**: 없음. 그냥 존재하면 됩니다.
...
| 스킬 | 한 줄 요약 | 호출 예시 |
|------|-----------|----------|
| `bootstrap` | 처음 세팅 | "bootstrap 실행해줘" |
...
```
1. npx k-harness init --ide vscode
2. "bootstrap 실행해줘" — 기존 코드를 스캔해서 상태 파일을 채워줌
