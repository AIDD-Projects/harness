...
> LLM이 프로젝트를 "이해"하고 "기억"하게 만드는 개발 하네스 프레임워크
...
| 유형 | 역할 | 위치 (VS Code) |
|------|------|----------------|
| **규칙** (Rules) | LLM이 매 요청마다 자동으로 따르는 규칙 | `.github/copilot-instructions.md`, `.vscode/instructions/` |
| **스킬** (Skills) | 사용자가 필요할 때 호출하는 절차 가이드 | `.github/skills/` |
| **상태 파일** (State) | 세션 간 프로젝트 맥락을 유지하는 문서 | `docs/` |

설치 직후:
```
"bootstrap 실행해줘" → 상태 파일 + 규칙 파일 자동 채우기 → 개발 시작
```

---

## 2. 핵심 개념: 스킬 vs 에이전트 vs 규칙

K-Harness에는 3가지 부품이 있습니다. **이 구분을 이해하면 언제 뭘 써야 하는지 명확해집니다.**

### 규칙 (Rules) — 자동. 사용자가 신경 쓸 필요 없음

LLM이 **매 요청마다 알아서** 읽는 파일입니다. 호출하지 않아도 적용됩니다.
`bootstrap`을 실행하면 프로젝트에 맞게 자동으로 채워집니다.

- `copilot-instructions.md` → Iron Laws, Direction Guard, Session Bootstrap, **Architecture, Directory Structure** (bootstrap이 채움)
- `testing.instructions.md` → 테스트 코드 작성 시 자동 적용 (글로브가 프로젝트 언어에 맞게 설정됨)
- `backend.instructions.md` → 백엔드 코드 작성 시 자동 적용 (글로브가 프로젝트 언어에 맞게 설정됨)

**사용자 할 일**: 없음. `init` + `bootstrap`이 알아서 채웁니다.
...
1. npx k-harness init --ide vscode
   → 프로젝트 언어를 자동 감지하여 글로브(Python/Go/Java 등)를 맞춰 설정
2. "bootstrap 실행해줘"
   → 프로젝트 스캔 + 6가지 질문 → 상태 파일 + 규칙 파일 모두 자동 채우기
...
```
1. npx k-harness init --ide vscode
   → 언어 자동 감지 (requirements.txt, go.mod 등)
2. "bootstrap 실행해줘" — 기존 코드를 스캔해서 상태 파일 + 규칙 파일 채우기
...
│         │                                           │
│         ▼                                           │
│  @planner ──→ 계획 수립 (feature-breakdown 포함)    │
│         │                                           │
│         ▼                                           │
│     [구현]                                          │
│         │                                           │
...
- K-Harness: ~20 파일, 8 스킬 → 저성능 LLM(GPT-OSS-120B 등)에서도 안정 동작
