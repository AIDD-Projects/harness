...
Musher(Execution 하네스)와 kode:crew(Planning 포털)를 통합하여 KT 사내 소프트웨어 개발 전 생명주기(구상→설계→개발→배포)를 하나의 프레임워크로 지원한다.

**단, 통합 이전에 Musher 자체의 실사용 검증이 선행되어야 한다.**
...
Musher의 가장 큰 약점은 기술이 아니라 **"실증"**이다.
...
**목표**: Musher를 사용하여 Musher 자체를 개발한다 (dogfooding).

**구체적 활동**:
1. Musher 리포에 `npx musher init --ide vscode` 실행
2. `docs/project-brief.md`를 Musher 자체 비전으로 채움
3. 이후 모든 Musher 기능 개발을 Musher 워크플로우로 진행
...
| 개발 속도 변화 | 측정만 | Musher 유/무 비교 (기준선 확보) |
...
**목표**: 나 이외의 개발자가 실제 프로젝트에서 Musher를 사용한다.
...
> **v0.9.0 활용**: 팀 파일럿에서 `npx musher init --team`으로 Team Mode 사용. 각 팀원의 State File 충돌 없이 협업 가능.

**파일럿 프로토콜**:
1. Musher 설치 + 30분 온보딩 세션
...
| # | 질문 | 응답 형태 |
|---|---|---|
| 1 | 이번 주 Musher를 몇 번 사용했나? | 숫자 |
| 2 | 가장 도움이 된 스킬/에이전트는? | 선택 |
| 3 | 가장 방해가 된 것은? | 서술 |
| 4 | Musher 없이 했으면 더 빨랐을 작업이 있었나? | Yes/No + 서술 |
| 5 | 계속 쓰고 싶은가? (1~5점) | 점수 |
...
**핵심 가설**: "다른 개발자도 Musher를 쓰면 개발 워크플로우가 개선된다"
...
**검증**: Gate 0과 동일한 dogfooding. Musher v0.7.0 개발에 Planning 스킬 적용하여 검증.
...
Musher (IDE) → project-brief.md를 입력으로 사용
```
...
### 시나리오 B: Musher가 Planning 흡수

```
Musher에 analyze + architect + arb-review 추가
kode:crew의 역할이 자연소멸
```
...
**권장: 시나리오 C 우선, B 대비.**
Musher 검증(Gate 0~1)이 선행되어야 어떤 시나리오든 논의 자격이 생김.
...
| R3 | kode:crew 팀이 통합을 거부 | 중 | 중 | 시나리오 A로 후퇴. Musher가 강해지면 자연 해결 |
| R4 | Planning 스킬 품질 부족 | 높 | 중 | Execution만으로도 가치 있음. Planning은 보너스 |
| R5 | KT 조직 내 채택 거부 | 중 | 높 | Push 대신 Pull 전략. 실적으로 증명 |
| R6 | BMAD가 급성장하여 Musher 불필요 | 낮 | 높 | Musher 차별점(저성능 LLM, KT 표준, 경량)이 다른 시장 |
...
최소 성공 (Minimum Viable Success):
  Musher Execution (v0.5~0.6)이 1개 팀에서 3개월 이상 지속 사용

기본 성공:
  Musher Full Lifecycle이 3개 팀에서 사용
...
_이 문서는 Musher Full Lifecycle 통합 전략의 로드맵입니다. v0.9.0 Team Mode 완료로 Gate 1 (팀 파일럿)의 기술적 전제 조건을 갖추었습니다._
