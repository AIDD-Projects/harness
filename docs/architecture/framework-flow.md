# kode:musher Framework Architecture

> v1.0.0 | kode:musher의 컴포넌트가 어떻게 연결되고, 데이터가 어떻게 흐르는지 정의한 문서.  
...
kode:musher는 5개 계층으로 구성된다. 아래 계층이 위 계층에 의존한다.
...
kode:musher의 핵심 설계 원칙: **모든 작업의 결과가 state 파일로 돌아온다.**
