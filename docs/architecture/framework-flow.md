# K-Harness Framework Architecture

> K-Harness의 컴포넌트가 어떻게 연결되고, 데이터가 어떻게 흐르는지 정의한 문서.  
...
K-Harness는 5개 계층으로 구성된다. 아래 계층이 위 계층에 의존한다.
...
Layer 1: Rules           — 항상 로드, LLM 행동 제약
```

| 계층 | 파일 | 활성화 | 역할 |
|------|------|--------|------|
| Rules | core-rules.md, testing-rules.md, backend-rules.md | 항상 (IDE가 로드) | LLM 행동 제약, Iron Laws 강제 |
...
| Skills | 8개 스킬 (.md) | 수동 호출 | 단일 절차 실행 |
| Agents | 3개 에이전트 (.md) | 수동 호출 | 스킬 조합 + 판단 |
...
| **pivot** | W | W | W | W | W |
| **investigate** | — | — | — | — | R |
| **feature-breakdown** | — | W | W | W | — |
| **impact-analysis** | — | — | W* | R/W | — |
| **test-integrity** | — | — | — | W* | R/W |
| **security-checklist** | — | — | — | — | W* |
| **reviewer** | — | R | R | R | R |
| **planner** | R | R/W | R/W | R | R |
| **sprint-manager** | R | R | R/W | — | R |
...
2. 수정 후 `test-integrity`가 mock 동기화 확인
3. `reviewer`가 전체 검증
...
**데이터 흐름:**
1. 세션 시작 시 `core-rules.md`의 New Session Bootstrap이 4개 state 파일 읽기 지시
...
**트리거:** core-rules.md의 Direction Guard가 매 요청 시 docs/project-brief.md를 확인.  
...
K-Harness의 핵심 설계 원칙: **모든 작업의 결과가 state 파일로 돌아온다.**
...
| 스킬 | 독립 실행 가능 | 에이전트에서 호출됨 | State 쓰기 |
|------|:---:|:---:|:---:|
| bootstrap | ✅ | — | ✅ (4개) |
| learn | ✅ | — | ✅ (3개 + memory) |
| pivot | ✅ | — | ✅ (5개) |
| investigate | ✅ | — | — |
| feature-breakdown | ✅ | planner | ✅ (3개) |
| impact-analysis | ✅ | planner, reviewer | ✅ 조건부 (dependency-map, project-state) |
| test-integrity | ✅ | reviewer | ✅ 조건부 (failure-patterns, dependency-map) |
| security-checklist | ✅ | reviewer | ✅ 조건부 (failure-patterns) |

**핵심 관찰:**
- `bootstrap`, `learn`, `pivot`은 **state 관리 스킬** — 직접 호출만 가능, 항상 state 쓰기
- `feature-breakdown`은 독립 + 에이전트 양쪽에서 사용, 항상 state 쓰기
- `impact-analysis`, `test-integrity`, `security-checklist`은 **분석 스킬** — 주로 읽기, 상황에 따라 조건부 쓰기
- `investigate`만 순수 읽기 전용
