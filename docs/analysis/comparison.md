# 경쟁 분석: BMAD vs gstack vs GSD vs kode:musher

> **목적**: kode:musher가 프로젝트 방향성 관리와 코드 품질 하네스에서 경쟁 프레임워크 대비 어떤 위치에 있는지를 지속적으로 추적하고, 부족한 부분을 식별하여 개선하기 위한 Living Document.  
> **최종 갱신**: 2026-04-09 (kode:musher v1.0.0)

---

## 1. 프로젝트 개요

|  | BMAD | gstack | GSD | kode:musher |
|---|---|---|---|---|
| GitHub | bmad-method | gstacmusher | get-shit-done | musher |
...
| 차원 | BMAD | gstack | GSD | kode:musher |
|---|---|---|---|---|
...
### 4.1 프로젝트 방향성 관리 (kode:musher 핵심 영역)

| 기능 | BMAD | gstack | GSD | kode:musher |
|---|---|---|---|---|
...
> **판정**: kode:musher가 유일하게 프로젝트 방향성 관리를 명시적 기능으로 제공. 다른 프레임워크는 프로세스 관리로 간접 방지하거나 아예 없음.
...
> **판정**: GSD가 압도적. 단, kode:musher는 프로세스 자동화가 목적이 아님 — "올바른 코드를 작성하도록 돕는 하네스"가 목적.
...
> **판정**: kode:musher가 가장 체계적. Iron Laws 8개는 다른 프레임워크에 없는 독자적 개념.
...
> **판정**: GSD와 kode:musher는 정반대 전략으로 같은 문제를 해결. GSD는 "더 많은 context를 제공" (fresh window), kode:musher는 "더 적은 context로 해결" (22줄 디스패처).
...
| 설치 복잡도 | npx bmad-method install | git clone && ./setup | npx get-shit-done-cc | npx musher init |
...
> **판정**: kode:musher가 유일하게 팀 협업 시 State file merge 충돌 문제를 해결. 다른 프레임워크는 1인 개발자 가정.
...
| 아이디어 | 채택 여부 | 근거 |
|---|---|---|
| Plan별 fresh context | ❌ 거부 | kode:musher는 context를 줄이는 전략. 반대 방향. |
...
## 7. kode:musher 고유 강점 (다른 프레임워크에 없는 것)
...
- Context 효율에서 압도적 우위 (BMAD 4~6파일, GSD agent 프롬프트 수천 줄 vs kode:musher 22줄)
...
## 8. kode:musher 보완 필요 영역 (Watchlist)

> 아래 항목은 "채택할 것"이 아니라 "경쟁 동향 모니터링 대상"입니다.

| 영역 | 현재 상태 | 모니터링 대상 | 비고 |
|---|---|---|---|
| 프로세스 자동화 | 범위 밖 | GSD의 discuss→plan→execute 자동화 | kode:musher 정체성과 충돌. 채택 안 함. |
...
### kode:musher가 경쟁하는 영역

> "LLM이 프로젝트 방향을 잃지 않고, 반복 실수 없이, 올바른 코드를 작성하도록 돕는 하네스"

이 영역에서 kode:musher는 BMAD, gstack, GSD 대비 명확한 우위:
