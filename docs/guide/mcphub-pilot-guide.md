...
핵심 컨셉 3가지:
- **디스패처**: LLM이 매 세션마다 자동으로 읽는 42줄 워크플로우 안내
- **Skills**: 특정 작업을 위한 단계별 절차 (디버깅, 코드리뷰, 보안점검, 의존성 분석 등)
- **Agents**: 역할 기반 페르소나 (reviewer, planner, sprint-manager, architect)
...
프로젝트/
├── .github/
│   ├── copilot-instructions.md         ← 42줄 디스패처 (매 세션 자동 적용)
│   ├── agents/
...
#### Claude Code 사용법
에이전트가 `.claude/skills/`에 스킬로 변환되어 있으므로 자연어로 요청:
```
"planner 모드로 기능 분석해줘"
...
#### Cursor 사용법
`.cursor/skills/planner/SKILL.md` 등이 생성되며, 필요 시 자연어로 요청합니다.
...
- **Claude Code**: 에이전트가 `.claude/skills/`에 스킬로 변환되므로, "planner 모드로 기능 분석해줘"처럼 자연어로 요청하세요.
- **Cursor**: `.cursor/skills/planner/SKILL.md`가 생성되며, 필요 시 자연어로 요청합니다.
...
| **Week 3** | 피드백 정리 + 제출 |
| **Week 4** | K-Harness 팀이 피드백 반영 → 다음 릴리즈 |

---

*K-Harness — "LLM이 같은 실수를 두 번 하지 않게"*
