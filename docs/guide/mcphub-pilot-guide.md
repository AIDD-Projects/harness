...
4. 각 Wave별로 구현 (의존성 순서대로)
   ↓
5. @reviewer가 리뷰 (dependency-map 업데이트 확인 포함)
   ↓
...
```
나: @planner SSE 연결 관리 기능을 추가하고 싶어. 클라이언트가 서버에 SSE로 연결하면
...
Iron Law #6이 있으므로 Copilot이 모듈 추가/수정 시 자동으로 dependency-map 업데이트를 제안합니다. 또한 `@reviewer`가 commit 전에 업데이트 누락을 감지합니다.
