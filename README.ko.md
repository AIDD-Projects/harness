...
Musher는 모든 개발자의 AI에게 동일한 목표·비목표·결정사항·프로젝트 상태를 제공하여, 누가 코딩하든 어떤 IDE를 쓰든 **하나의 방향**으로 수렴하게 합니다.

## 핵심 설계 원칙

> **경량성**: Claude Sonnet이나 GPT-4o 같은 대형 LLM뿐 아니라, 프라이빗 망의 소형 LLM(GPT OSS 120B급, 로컬 LLM 등)도 Musher 파일을 읽고 프로젝트 방향을 유지할 수 있어야 합니다. BMAD의 200개 이상 파일 대비 **~25개 파일, ~17K 토큰**으로 동일한 효과를 달성합니다.
...
- **Core Rules** — 42줄 디스패처: 세션 시작 가이드, 워크플로 참조, state 파일 목록, Iron Laws. 상세 규칙은 각 스킬/에이전트에 임베딩되어 있습니다.

### 스킬 (온디맨드 절차)

| 스킬 | 설명 |
|------|------|
| **bootstrap** | 프로젝트를 Musher에 온보딩: 코드베이스 스캔 + state 파일 자동 작성 |
...
`musher init` 후 `bootstrap` 스킬을 실행합니다. 코드베이스를 스캔하고, 목표/비목표에 대해 질문한 후, 5개 state 파일을 자동으로 채웁니다. **가장 중요한 단계입니다** — 이것 없이는 Direction Guard 등 다른 스킬이 컨텍스트를 가지지 못합니다.
...
**Musher Engineering이 빛나는 부분입니다.** 여러 개발자가 각자 AI 세션을 실행하면 방향 이탈은 필연적입니다 — 공유 가드레일이 없다면.
...
8개 규칙이 모든 스킬과 에이전트에 적용됩니다. Musher로 관리되는 프로젝트의 품질 근간을 형성합니다.
...
Musher는 **AI가 어디로 가는지** — 모든 개발자의 AI가 같은 방향으로 움직이는지에 집중합니다.
...
| 태스크당 컨텍스트 | 4-6 파일 | 1 파일 | 매번 200k 플랜 | **2-3 파일 (42줄 디스패처)** |
...
- [ ] 자동 Pre-Pull 검증을 위한 Git hook 통합
- [ ] PR에서 Direction Guard 위반을 탐지하는 CI 파이프라인
- [ ] 기업 팀 도입 실제 사례 연구
- [ ] Team Mode용 자동 state 파일 충돌 해결
- [ ] 성능 벤치마크: Musher 적용 전후 비교
