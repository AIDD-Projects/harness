...
kode:harness는 현재 **v0.9.4** — 6개 IDE 어댑터 공식 문서 정합(Antigravity `.agents/`, Codex `.toml`, Cursor `.cursor/rules/`), state-check 스킬, Iron Law #10 (Self-Verify), Confirmation Gate Defaults, Crew 모드용 CI Artifact Index.
...
| **IDE Realignment** | v0.9.4 | ✅ 현재 | 6개 IDE 어댑터 공식 문서 정합; Antigravity `.agents/`, Codex `.toml`, Cursor `.cursor/rules/`; release 스킬 Step 6.5 + qa-check.sh §10 회귀 가드 |
