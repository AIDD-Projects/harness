...
kode:harness는 현재 **v0.9.7** — `harness/` ↔ `.github/` drift 가드 (`npm run harness:check-drift`), kode:vibe 에코시스템 위치 안내 + IDE 선택 가이드, project-brief.md 구체 예시를 추가했습니다. v0.9.6은 init 백업, pm 네이밍 정렬, LICENSE 정리입니다.
...
| **Drift Guard & Positioning** | v0.9.7 | ✅ 현재 | `harness/`↔`.github/` drift 가드, reviewer working-proof 게이트, kode:vibe 위치 안내, IDE 선택 가이드, project-brief 예시 |
